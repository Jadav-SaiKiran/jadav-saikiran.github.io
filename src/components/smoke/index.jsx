...
const JaiDeepSmoker = () => {
  ...
  useEffect(() => {
    ...
    var clearShader = compileShader(
      ...
      void main () {
        gl_FragColor = vec4(value * texture2D(uTexture, vUv).rgb, 1.0); 
      }`
    );
    ...
    var advectionShader = compileShader(
      gl.FRAGMENT_SHADER,
      `precision highp float; precision mediump sampler2D; varying vec2 vUv; uniform sampler2D uVelocity; uniform sampler2D uSource; uniform vec2 texelSize; uniform float dt; uniform float dissipation; void main () {     vec2 coord = vUv - dt * texture2D(uVelocity, vUv).xy * texelSize;     gl_FragColor = vec4(dissipation * texture2D(uSource, coord).rgb, 1.0); }`
    );
